...
export type VisitDay = { key: string; short: string; label: string; count: number };

// Igual que SalesChart (Ventas), pero para un solo número por día (visitas), sin importe.
...
export default function VisitsChart({ days }: { days: VisitDay[] }) {
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 8, paddingTop: 22 }}>
        <div aria-hidden="true" style={{ width: 28, height: 140, position: 'relative', flexShrink: 0 }}>
          {ticks.map((t) => (
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div role="img" aria-label={`Visitas por día. Máximo ${days[peak].count} el ${days[peak].label}.`} style={{ position: 'relative', height: 140 }}>
            {ticks.map((t) => (
            ...
            <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'flex-end' }}>
              {days.map((d, i) => (
                <div
                  key={d.key}
                  tabIndex={0}
                  aria-label={`${d.label}: ${d.count} ${d.count === 1 ? 'visita' : 'visitas'}`}
            ...
            {tip && (
              ...
              >
                <div style={{ fontSize: 15, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  {tip.count} {tip.count === 1 ? 'visita' : 'visitas'}
                </div>
